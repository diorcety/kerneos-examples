...
public class ModifyTargetCommand implements ICommand, IResponder {

    public function ModifyTargetCommand():void {}
    ...
    public function execute(event:CairngormEvent):void {
        ...
        model.getDelegate().modifyTarget((event as TargetCommandEvent).probetarget);
    }
    ...
    public function result(event:Object):void {
        var model:ProbeManagerModelLocator = ProbeManagerModelLocator.getInstance();
        var targets:ArrayCollection = (((event as ResultEvent).result) as ArrayCollection);
        if (targets != null) {
            // we must cast in ProbeTarget !
            var tmp:ArrayCollection = new ArrayCollection();
            for each (var p:ProbeTarget in targets) {
                tmp.addItem(p);
            }
            model.targets = tmp;
        }
